Feature: Add Category
  ...
  Scenario: Add a valid category as Admin
    Given I login as "admin" with password "password" with role "ROLE_ADMIN" in order to add a category
  ...
  Scenario: Attempt to add a category as a normal user
    Given I login as "user" with password "password" with role "ROLE_USER" in order to add a category
